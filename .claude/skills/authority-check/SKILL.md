...
description: Before a session logs, acts on, or presents something as a ruling, decision, or ratified state, check whether that authority actually belongs to the session or belongs to Kevin alone. Use when asked "whose call is this," "run an authority check," "am I allowed to rule on this," "is this Kevin's fork or mine," or before writing any mark, decision log, or status change that reads as settled — especially base-fork choices, medium/architecture decisions, or anything that could later read as "already decided" when it was actually a session's own judgment call. Also fires on any rule, gate, or law that would end up giving Kevin less than he asked for — that misapplication is flagged regardless of how well-reasoned the rule sounds. Born from nesi/mind/LEARNED.md laws 20 and 25 — a session ruling its own base fork doesn't become lawful by being carefully reasoned, and no law is allowed to become a lever that reduces what Kevin gets.
---
...
"carefully," which Kevin reverted in four words the one time it happened.
The other is larger and easier to miss — a law or gate, applied in good
faith, that ends up giving Kevin less than what he actually asked for. Both
...
extending) later are Kevin's regardless of how confidently the session can
...
### 3. Check whether a rule here would give Kevin less than he asked for

If this is a gate, law, filter, or constraint firing to block, delay, or
water down something Kevin explicitly asked for: that is a misapplication
...
Kevin's words behind it, or is the session inferring "he'd probably want
...
- **KEVIN'S CALL** — the decision is a base fork, medium choice, or would
...
- **RULE MISAPPLIED** — a gate or law is firing against something Kevin
...
Kevin crosses) from a different angle — a swarm read across the corpus
...
its own: a source can quote Kevin's actual words and still get used to
