...
store), start from genuinely empty — not "empty in the parts I remember to
...
actually type — not a known-good test string chosen because it exercises the
right code path.
...
- Does every act the surface invites visibly change something, at the moment
  it happens — not eventually, not only in storage?
- Does every control name itself, on its own face or a caption that's
  actually on screen — never a neighboring label doing the naming for it?
...
- Does every distinct room/state/screen name itself, and name itself
  correctly — not the name of the place the hand just left?
...
evidence the second party judges — not a summary of your impression of the
...
stop the authoring session from typing the word WALKABLE itself — a prose
...
walk, so that the handoff in step 6 has something concrete to judge — the
...
a false empty result in that state — this was mistaken for a real blank
...
- **WALKABLE** — quote the outside party's own words, from the transcript
  artifact in step 5, not from memory of what they said. Never write this
  yourself.
- **BLOCKED** — a hard failure the pre-check or the walk actually found; name
  ...
  broken.
- **UNWITNESSED** — the honest default whenever no non-authoring party has
