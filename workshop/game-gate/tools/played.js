#!/usr/bin/env node
/*
 * PLAYED — the row the keeper's hand writes.
 *
...
if (has("--stop")) {
  append({ ts: new Date().toISOString(), event: "stop", source: "the keeper" });
  console.log("[play] stop row written. No phase starts after this until a later play row.");
  process.exit(0);
}

const word = val("--word");
const row = { ts: new Date().toISOString(), event: "played", source: "the keeper" };
