 ...
 *
 * Built 2026-08-13 on the keeper's mark "build the runner," from the four counsel
...
const WATER = path.join(G2D, "keepers-water.json");
...
if (!waterBefore) { say("[runner] BLOCKED — keepers-water.json not found; a build that drops the writing fails its own walk (S4)."); process.exit(1); }
fs.mkdirSync(NIGHT, { recursive: true });
