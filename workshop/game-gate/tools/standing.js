 ...
 * standing.
 *
 * the keeper's mark, 2026-08-17: "build the truth-bearing spine first, not TANK."
...
const KINDS = {
  ...
  INTENDED: {
    ...
    note: "ADMITTED 2026-08-17 on the keeper's mark, and it is the half of that mark that keeps it from being a lever. A law that only refuses the overstatement leaves an intention with nowhere lawful to sit, so the next session files it under a kind that flatters it. This kind is where an intention goes. It is not a lesser claim — the corpus's own 'held is lawful' says an unbuilt thing is a correct state — it is a claim whose supports establish that it was DECLARED and make no claim that it holds."
  },
  INFERRED: {
...
function evaluateAll(claims) {
  ...
  function evaluate(c) {
    ...
    if (!supports.length) {
      ...
      /* ── THE TENSE LINT ────────────────────────────────────────────────────
         The keeper's mark, 2026-08-17: admit this sentence to the spine —
