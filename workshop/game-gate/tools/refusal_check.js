...
/* THIS LIST WENT STALE AND A BREACH RODE THROUGH IT, 2026-08-16.
   ...
   and that is the keeper's ruling, not a build session's.
   NOT ADDED, AND WHY: options.html fails this check on the single token
   "score", which occurs there only in the keeper's own reasoning ABOUT law 2 on a
...
/* day_one.html, descent.html, traversal.html DROPPED 2026-08-26, the keeper's
   ruling (asked directly, per this file's own header: "closing it means
   deciding what counts as a player surface, and that is the keeper's ruling,
...
const groups = {
  ...
  /* ═══ ADDED 2026-08-17 ON THE KEEPER'S F4 MARK ════════════════════════════════
...
/* ═══ THE CORE LOOP, DEFINED IN A WAY THAT HOLDS ═════════════════════════════
   The keeper's F9 mark, 2026-08-17: "the last twelve are outside the core loop by
...
if (missing.length) {
  ...
  if (outside) {
    ...
    console.log("        law 4/5 and the model call lifted here on the keeper's F9 mark;",
