#!/usr/bin/env python3
"""
marks.py — the fail-safe brake + the floor indicator for the keeper's marks.
...
The author puts this line in L0 of every widget so the keeper never guesses whether a mark took.

Scope: writes only MARKS_LOG.jsonl (a peer ledger). NEVER patterns/ or crossing records.

THE ACT VALVE (added 2026-07-30, the keeper's mark — "build the valve"): the log had a
...
Usage:
  python tools/marks.py catch --mark "PROMOTE: X" --source the keeper [--acted "crossed X"]
  ...
  python tools/marks.py act --batch overlays.jsonl   # many closures at once (the keeper-marked batches)
"""
...
def _parse_edges(vals):
    ...
    The named form is the one that matters (the keeper's naming 2026-08-06: the relation
...
# --------------------------------------------------------------------- compost
# the keeper's mark, 2026-08-06: "extend the compost window to marks too."
#
