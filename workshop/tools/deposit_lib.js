...
// this corpus holds. "Not to be routed" (the keeper's mark, 2026-08-31) means
...
// De-naming at the crossing (the keeper's mark, 2026-08-31: "lets remove the keeper
...
function denamePublicText(text) {
  ...
  t = t.replace(/https?:\/\/(?:www\.)?the keeper\.substack\.com[^\s)\]`'"<>]*/gi,
    "(the keeper's published essay — URL withheld with the name)");
  t = t.replace(/the keeper/gi, 'the keeper');
  t = t.replace(/the keeper[-_]?lens/gi, 'a held-back lens agent');
  t = t.replace(/keepers-water/gi, 'keepers-water'); // filename form, coupled to game2d's data file
  t = t.replace(/the keeper/g, 'the keeper');
  t = t.replace(/the keeper/gi, 'the keeper'); // the surname, any embedding — boundary-free so the shipped copy of this very line cleans itself
  t = t.replace(/THE KEEPER'S/g, "THE KEEPER'S").replace(/THE KEEPER/g, 'THE KEEPER');
  t = t.replace(/the keeper's/g, "the keeper's").replace(/the keeper's/g, "the keeper's");
  t = t.replace(/the keeper/g, 'the keeper').replace(/\bkeeper\b/g, 'the keeper');
  t = t.replace(/\bkeepers\b/gi, "the keeper's");
  ...
  t = t.replace(/the keeper/gi, 'keeper');
  ...
  t = t.replace(/C:\\+Users\\+<user>\\+(?:OneDrive\\+Desktop\\+DSS content|dev\\+DSS content)/g, '<corpus-root>');
  t = t.replace(/C:\/Users\/<user>\/(?:OneDrive\/Desktop\/DSS content|dev\/DSS content)/g, '<corpus-root>');
  t = t.replace(/C:\\+Users\\+<user>/g, '<home>');
  t = t.replace(/C:\/Users\/<user>/g, '<home>');
  t = t.replace(/C--Users-<user>-(?:OneDrive-Desktop|dev)-DSS-content/g, '<corpus-slug>');
  t = t.replace(/<user>/g, '<user>');
  // Re-capitalize at sentence and line starts.
  t = t.replace(/(^|[.!?]\s+|\n\s*)the keeper/g, (m, p) => p + 'The keeper');
  return t;
}

// The sublimation stage (the keeper's mark, 2026-09-01: "do the same
...
// creation before use" (the keeper's mark). Exclusions are counted and named
...
// for the first entry and why (the keeper's mark, 2026-08-31: consent
...
// the keeper's felt read is the authority, this never is. See
...
// tools/agent_invocation_check.js (built 2026-08-31 on the keeper's own catch:
