...
**Drafted:** 2026-07-16 · NESI memory-migration step 5 · **Status: RATIFIED — the keeper's mark 2026-07-16, "ratify as written." This file is NESI-side authority for vocabulary; questions resolve here first.**
...
### mark
- **IS:** a human decision, typed by the keeper. The only thing that ratifies, promotes, deletes, or publishes anything. AI never marks.
...
- **IN ARTIFACTS:** every mark is recorded with date + the keeper's words when available ("the keeper's mark: 'compost the other two'"). A document that changed state must show which mark changed it.
...
- **IN ARTIFACTS:** staged documents say so in their status line ("STAGED — awaiting the keeper's mark") and never describe themselves as decided.

### cross / the membrane
- **IS:** the one-way passage between private material and the public-facing library (canon). Crossing is a ceremony the keeper performs; it cannot be queued, batched, or automated. The Membrane Controller holds exclusive write authority at the two crossing points.
...
- **IN ARTIFACTS:** the compost line states the reason in plain words, ideally the keeper's own ("the keeper's one-line falsifier: 'would the keeper actually use this? no.'").
...
### uncross *(fourth verb — ratified 2026-07-19, the keeper's mark: "keep undo, update the rules")*
- **IS:** the keeper's one-motion rollback of a mistaken cross — the pattern folds to `patterns/_folded/` (kept, never deleted), the index rebuilds sync-loud or the fold reverts, and the ledger records `uncross`. The keeper-only, like every mark.
- **IS NOT:** an automatic or engine-initiated reversal, a soft delete, or "contradiction." It fires only on the keeper's touch, only on a crossed object, and leaves the full trail.
...
- **IS:** the library of ~90 ratified, transferable patterns — the flat reference surface all new work is checked against. The keeper's figure: a machinist's flat table, precision overbuilt in, that other tools get built and checked against.
...
### the gate
- **IS:** the queue of staged items awaiting the keeper's marks, plus its data file (`gate/data/gate_data.json`). The gate is human-only territory: no approve button, no auto-mark, ever.
...
### felt-read
- **IS:** a somatic yes/no the keeper performs. Deliberately not automatable. Instruments exist to *prompt* it (body-question composer, mark-record's field 5); none exist to *pass* it, simulate it, or infer it.
...
- **IN ARTIFACTS:** the felt-read field appears blank unless the keeper filled it. A blank field is a complete record, not a missing one.

### organ
- **IS:** a named function of the system — metabolizer (sorts raw piles into dispositions), substrate (extracts patterns from writing), Converger (routes promote/hold/compost), Governor (the brake on the keeper's own overwork), daily-cycle (session open/close ritual), and peers. Organs are anatomy.
...
3. **Mark-reasoning inline** — when a mark changed this document's state, the mark's date and (when available) the keeper's words appear at the point of change, not in a separate log.
...
---
*Ratified as written, the keeper's mark 2026-07-16. This file stands with PROTOCOLS.md and reflexes.md as NESI-side authority; future vocabulary questions resolve here first. Amendments enter the way this did: drafted, then marked.*
