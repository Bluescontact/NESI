#!/usr/bin/env node
/* ═══ THE CROSSING AUDIT ═══════════════════════════════════════════════════════
 * Built 2026-08-16 on Kevin's order: "i want the 176 run against the geometry
 ...
 * his poured sentences from kevins-water.json, the marks he authored in
...
try {
  const w = JSON.parse(fs.readFileSync(path.join(ROOT, "nesi/game2d/kevins-water.json"), "utf8"));
  ["tank", "grounds", "lake", "compost", "linesList", "standing"].forEach(k =>
    (Array.isArray(w[k]) ? w[k] : []).forEach(e => addProbe(e && (e.text || e.t || e), "water")));
} catch (e) { console.error("crossing_audit: could not read kevins-water.json — " + e.message); process.exit(1); }

/* his own marks */
try {
  fs.readFileSync(path.join(ROOT, "MARKS_LOG.jsonl"), "utf8").split("\n").forEach(l => {
    if (!l.trim()) return;
    try { const r = JSON.parse(l); if (r.source === "kevin") addProbe(r.mark, "mark"); } catch {}
  });
} catch {}

/* THE CORPUS — NESI.html at the repo root, added 2026-08-16 on Kevin's order
...
L("  of what may leave is Kevin's, and this instrument does not make it.");
L("");
