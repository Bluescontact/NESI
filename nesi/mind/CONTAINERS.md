...
**Adopted 2026-08-03 on Kevin's mark**, three marks in one deposit, caught in `MARKS_LOG.jsonl`:
...
- The eight candidate membrane sentences (08-02), marked for by Kevin on 08-03 and **gone** — they lived in a session transcript and nowhere else.
...
### The deposit-before-render rule (Kevin's mark, 2026-08-04)
...
`DECISIONS_OFFERED.jsonl` is the EVENT container that clause needed. Every decision put in front of Kevin is deposited there **before** the surface showing it is written, via `tools/decisions.py offer`. A surface is where a decision is *shown*; it was never supposed to be where a decision is *kept*.
...
**An offer is not a mark.** It records that a choice was put in front of Kevin, never that he made one. Marks stay in `MARKS_LOG.jsonl` and only his hand puts them there. The two ledgers together answer a question neither could alone — `decisions.py open` lists every decision ever offered and never answered, which is exactly the load the stranded widget files used to leave in Kevin's head.
...
| **DECIDE** | Kevin's hands | substrate → dispositioned |
| **CROSS** | Kevin's hands, gated | dispositioned → canon, then canon → released |
...
|---|---|---|---|---|---|
| 1 | **raw** | dropped, unread, unsorted | Kevin drops · harvest | SURFACE | `_INTAKE/` — shared with 2, 3, 4 |
...
| 4 | **dispositioned** | carries Kevin's mark: promote-ready / hold / compost | DECIDE | CROSS | **no container** — dispositions live *inside* the briefs |
...
Columns: `path | kind | state | in-action | out-action | derived_from | mirror_of`. Use `-` for not-applicable, and `?` for **unruled** — a container whose kind has not been decided. Unruled is an honest state and the checker reports it as an open question rather than a breach; it is Kevin's ruling to make, not the machine's to infer. A row may omit the seventh column, which reads as `-`.

### Law 4 — a duplicate declares which copy wins (Kevin's mark, 2026-08-06)
...
**A mirror is not a rendering — the fourth pair is declared differently, and this is a deliberate departure from the mark.** Kevin's mark named `ESCALATION_CONDITIONS` json → `ROUTER.md` as one of the four pairs. But `mirror_of` means *should be byte-identical*, and `ROUTER.md` is a **rendering** of a json block as a human-readable table — it is not supposed to match byte for byte, and declaring it a mirror produced an immediate false positive on the first run. It is declared `derived_from` instead. The authority relationship Kevin wanted is stated exactly as before, in ROUTER.md's own prose, which is where it was already working. **Reversible on his word** — if he wants it declared as a mirror anyway, the row changes back and the check will report it drifted every run.
...
**A finding the registry itself surfaces, recorded rather than smoothed over:** `_COMPOST/` (129 files) and `nesi/_compost/` (33 files) hold material in a state the six-state chain does not name. Compost is a DECIDE *disposition*, not a state in the CROSS chain — yet it has a container in active use. They are declared here with state `compost` so the checker reports the mismatch instead of hiding it. Under Law 3's own falsifier this is the signal to either recut the chain to seven states or reclassify compost as a disposition whose material stays where it is. **Unruled — Kevin's call, not the machine's.**
...
- **Retiring the nine unfilled `nesi/` containers.** Offered, unmarked. All nine still stand. The file system cannot tell a dead folder from a waiting intention, and neither can the machine — only Kevin can.
- **A checker that enforces any of this.** Offered, unmarked. See below.

---

## Enforcement — built 2026-08-03 on Kevin's mark
...
- **OPEN (exit 0, reported loudly)** — `UNRULED`, a container whose kind has never been decided, and `WISH`, one named but never filled. These are Kevin's rulings. **The machine will not infer a kind**, which is why 42 containers sit unruled rather than being classified by guess.
